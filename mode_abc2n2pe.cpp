...
mode_abc2n2pe::mode_abc2n2pe()
{

}

void mode_abc2n2pe::method_calc(QStringList sl_jiahao, QMap<QString, QString> map)
{
    ...
    sl_content.append(QString::number(pabc_width)+"*"+QString::number(pabc_deep)+"+"+QString::number(pn_sl)+"*"+QString::number(pn_width)+"*"+QString::number(pn_deep)+"+"+QString::number(ppe_sl)+"*"+QString::number(ppe_width)+"*"+QString::number(ppe_deep));

    if(state=="t"){
       sl_status.append("ABC"+QString::number(pn_sl)+"N"+QString::number(ppe_sl)+"PE MODE | 单排复零复地 材质 铜");
    ...
    if(state=="l"){
       sl_status.append("ABC"+QString::number(pn_sl)+"N"+QString::number(ppe_sl)+"PE MODE | 单排复零复地 材质 铝");
    ...
    PAI="Pabc*3*"+PAICONTENT +"+Pn*"+QString::number(pn_sl)+"*"+PAICONTENT+"+Ppe*"+QString::number(ppe_sl)+PAICONTENT;

    sl_content.append(PAI);
    SUMP=Pabc*3*PAICONTENTD +Pn*pn_sl*PAICONTENTD+ Ppe*ppe_sl*PAICONTENTD;

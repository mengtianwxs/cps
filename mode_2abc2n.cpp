...
mode_2abc2n::mode_2abc2n()
{

}

void mode_2abc2n::method_calc(QStringList sl_jiahao, QMap<QString, QString> map)
{
   ...
   pabc_sl=sl_xinghaoabc.at(0).toInt();
    ...
    sl_content.append(QString::number(pabc_sl)+"*"+QString::number(pabc_width)+"*"+QString::number(pabc_deep)+"+"+QString::number(pn_sl)+"*"+QString::number(pn_width)+"*"+QString::number(pn_deep));

    if(state=="t"){
       sl_status.append(QString::number(pabc_sl)+"ABC"+QString::number(pn_sl)+"N MODE | 复排复零 材质 铜");
    ...
    if(state=="l"){
       sl_status.append(QString::number(pabc_sl)+"ABC"+QString::number(pn_sl)+"N MODE | 复排复零 材质 铝");
    ...
    PAI="Pabc*3*"+QString::number(pabc_sl)+"(0.6*"+str_t600num+
            "+0.8*"+str_t800num+
            "+1*"+str_t1000num+
            "+1.2*"+str_t1200num+
            "+0.4*"+str_t400num+"+"+
    ...
    SUMP=Pabc*3*pabc_sl*(0.4*t400_num+0.6*t600_num+0.8*t800_num+1*t1000_num+1.2*t1200_num+tfb1_width*tfb1_num+tfb2_width*tfb2_num+tfb3_width*tfb3_num)+

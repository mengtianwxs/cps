...
mode_2abcnpe::mode_2abcnpe()
{

}

void mode_2abcnpe::method_calc(QStringList sl_jiahao, QMap<QString, QString> map)
{
   ...
   sl_xinghaope=ppe.split("*");
    ...
    sl_content.append(QString::number(pabc_sl)+"*"+QString::number(pabc_width)+"*"+QString::number(pabc_deep)+"+"+QString::number(pn_width)+"*"+QString::number(pn_deep)+"+"+QString::number(ppe_width)+"*"+QString::number(ppe_deep));

    if(state=="t"){
        sl_status.append(QString::number(pabc_sl)+"ABCNPE MODE | 复排单零单地 材质 铜");
    ...
    if(state=="l"){
        sl_status.append(QString::number(pabc_sl)+"ABCNPE MODE | 复排单零单地 材质 铝");
    ...
    PAI="Pabc*3*"+QString::number(pabc_sl)+"*"+PAICONTENT +"+Pn*"+PAICONTENT+"+Ppe*"+PAICONTENT;

   sl_content.append(PAI);
    SUMP=Pabc*3*pabc_sl*PAICONTENTD +Pn*PAICONTENTD+ Ppe*PAICONTENTD;

...
mode_abcn2pe::mode_abcn2pe()
{

}

void mode_abcn2pe::method_calc(QStringList sl_jiahao, QMap<QString, QString> map)
{
    initMap(map);
    pabc=sl_jiahao.at(0);
    pn=sl_jiahao.at(1);
    ...
    sl_content.append(QString::number(pabc_width)+"*"+QString::number(pabc_deep)+"+"+QString::number(pn_width)+"*"+QString::number(pn_deep)+"+"+QString::number(ppe_sl)+"*"+QString::number(ppe_width)+"*"+QString::number(ppe_deep));

    if(state=="t"){
        sl_status.append("ABCN"+QString::number(ppe_sl)+"PE MODE | 单排单零复地 材质 铜");
    ...
    if(state=="l"){
        sl_status.append("ABCN"+QString::number(ppe_sl)+"PE MODE | 单排单零复地 材质 铝");
    ...
    PAI="Pabc*3*"+PAICONTENT +"+Pn*"+PAICONTENT+"+Ppe*"+QString::number(ppe_sl)+"*"+PAICONTENT;

    sl_content.append(PAI);
    SUMP=Pabc*3*PAICONTENTD +Pn*PAICONTENTD+ Ppe*ppe_sl*PAICONTENTD;

...
mode_2abc2n2pe::mode_2abc2n2pe()
{

}

void mode_2abc2n2pe::method_calc(QStringList sl_jiahao, QMap<QString, QString> map)
{
    initMap(map);
    pabc=sl_jiahao.at(0);
    pn=sl_jiahao.at(1);
   ...
   sl_xinghaoabc=pabc.split("*");
    ...
    sl_content.append(QString::number(pabc_sl)+"*"+QString::number(pabc_width)+"*"+QString::number(pabc_deep)+"+"+QString::number(pn_sl)+"*"+QString::number(pn_width)+"*"+QString::number(pn_deep)+"+"+QString::number(ppe_sl)+"*"+QString::number(ppe_width)+"*"+QString::number(ppe_deep));

    if(state=="t"){
        sl_status.append(QString::number(pabc_sl)+"ABC"+QString::number(pn_sl)+"N"+QString::number(ppe_sl)+"PE MODE | 复排复零复地 材质 铜");
    ...
    if(state=="l"){
        sl_status.append(QString::number(pabc_sl)+"ABC"+QString::number(pn_sl)+"N"+QString::number(ppe_sl)+"PE MODE | 复排复零复地 材质 铝");
    ...
    PAI="Pabc*3*"+QString::number(pabc_sl)+"*"+PAICONTENT +"+Pn*"+QString::number(pn_sl)+"*"+PAICONTENT+"+Ppe*"+QString::number(ppe_sl)+PAICONTENT;

    sl_content.append(PAI);
    SUMP=Pabc*3*pabc_sl*PAICONTENTD +Pn*pn_sl*PAICONTENTD+ Ppe*ppe_sl*PAICONTENTD;

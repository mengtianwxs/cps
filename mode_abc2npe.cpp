...
mode_abc2npe::mode_abc2npe()
{

}

void mode_abc2npe::method_calc(QStringList sl_jiahao, QMap<QString, QString> map)
{
   ...
   pn_width=sl_xinghaon.at(1).toInt();
    ...
    sl_content.append(QString::number(pabc_width)+"*"+QString::number(pabc_deep)+"+"+QString::number(pn_sl)+"*"+QString::number(pn_width)+"*"+QString::number(pn_deep)+"+"+QString::number(ppe_width)+"*"+QString::number(ppe_deep));

    if(state=="t"){
        sl_status.append("ABC"+QString::number(pn_sl)+"NPE MODE | 单排复零单地 材质 铜");
    ...
    if(state=="l"){
        sl_status.append("ABC"+QString::number(pn_sl)+"NPE MODE | 单排复零单地 材质 铝");
    ...
    PAI="Pabc*3*"+PAICONTENT +"+Pn*"+QString::number(pn_sl)+"*"+PAICONTENT+"+Ppe*"+PAICONTENT;

    sl_content.append(PAI);
    SUMP=Pabc*3*PAICONTENTD +Pn*pn_sl*PAICONTENTD+ Ppe*PAICONTENTD;
